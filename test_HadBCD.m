    ...
    [X_FS,Y_FS,U_FS,V_FS,err_FS]=Had_BCD(Atrue,opts);
    fin_err_FS=err_FS(end);
    opts.init='Wertz';
    [X_Wertz,Y_Wertz,U_Wertz,V_Wertz,err_Wertz]=Had_BCD(Atrue,opts);
    fin_err_Wertz=err_Wertz(end);
    opts.init='FS2';
    tic;
    [X_FS2,Y_FS2,U_FS2,V_FS2,err_FS2]=Had_BCD(Atrue,opts);
    t_Sam=toc;
    fin_err_FS2=err_FS2(end);
    opts.init='FS3';
    [X_FS3,Y_FS3,U_FS3,V_FS3,err_FS3]=Had_BCD(Atrue,opts);
    ...
    % for i=1:n
    %     for j=1:m
    %         a=P(i,1:r)'; b=P(i,r+1:end)';
    %         c=Q(j,1:r)'; d=Q(j,r+1:end)';
    %         alpha=a'*c; beta=b'*d;
    %         XY(i,j)=0.5*(sqrt((1+2*alpha)*(1+2*beta))-sqrt((1-2*alpha)*(1-2*beta)));
    %         UV(i,j)=0.5*(sqrt((1+2*alpha)*(1+2*beta))+sqrt((1-2*alpha)*(1-2*beta))); 
    %     end
    % end
    % %norm(xy-X_FS3(i,:)*Y_FS3(j,:)')
    % %[rank(XY),rank(UV)]
    % 
    % X=X_Wertz; Y=Y_Wertz; U=U_Wertz; V=V_Wertz;
    % M=((X-Y)*(X+Y)').*((U+V)*(U+V)')-((X*X').*(U*U'))+((Y*Y').*(V*V'));