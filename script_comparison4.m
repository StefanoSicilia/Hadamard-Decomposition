%% Script to compare Had_eBCD and Had_manopt3
...
%%
    ...
    opts.maxtime=t_Wsvd;
    tic;
    [X1_Msvd,X2_Msvd,err_Msvd]=Had_manopt3(X,opts);
    t_Msvd=toc;
    fin_err_Msvd=err_Msvd(end);
    opts.init='FS'; 
    opts.maxtime=t_WFS;
    tic;
    [X1_MFS,X2_MFS,err_MFS]=Had_manopt3(X,opts);
    t_MFS=toc;
    fin_err_MFS=err_MFS(end);
    opts.init='FS2';
    opts.maxtime=t_WFS2;
    tic;
    [X1_MFS2,X2_MFS2,err_MFS2]=Had_manopt3(X,opts);
    t_MFS2=toc;
    fin_err_MFS2=err_MFS2(end);
    opts.init='FS3';
    opts.maxtime=t_WFS3;
    tic;
    [X1_MFS3,X2_MFS3,err_MFS3]=Had_manopt3(X,opts);
    t_MFS3=toc;
    ...
    Ur=U(:,1:r2)*Sr;
    Vr=V(:,1:r2)*Sr;
    fin_errSVDmatlab=0.5*norm(X-Ur*Vr','fro')^2;
    ...
    if fin_err_Msvd<1e5
        semilogy(err_Msvd,'bo--','LineWidth',lw)
        hold on
        legendlabel=[legendlabel,'Msvd'];
    end
    if fin_err_MFS<1e5
        semilogy(err_MFS,'co--','LineWidth',lw)
        hold on
        legendlabel=[legendlabel,'MFS'];
    end
    if fin_err_MFS2<1e5
        semilogy(err_MFS2,'o--','Color',[0.5,0.5,0.5],'LineWidth',lw)
        hold on
        legendlabel=[legendlabel,'MFS2'];
    end
    if fin_err_MFS3<1e5
        semilogy(err_MFS3,'ko--','LineWidth',lw)
        hold on
        legendlabel=[legendlabel,'MFS3'];
    end
    if fin_errSVDmatlab<1e5
        semilogy(fin_errSVDmatlab,'o--','Color',[0.75,0.5,0],'LineWidth',lw)
        hold on
...
%%
    if imageflag
        sq=255;
        figure(2)
        imshow(sq*(X_WFS*Y_WFS').*(U_WFS*V_WFS'))
        title('BCD')
        figure(3)
        imshow(sq*prodsvd(X1_MFS).*prodsvd(X2_MFS))
        title('Manifold')
        figure(4)        
        imshow(255*Ur*Vr')
        title('SVD')
        ...
        sW1=ssim(X,(X_WFS*Y_WFS').*(U_WFS*V_WFS'));
        sM1=ssim(X,prodsvd(X1_MFS).*prodsvd(X2_MFS));
    end
