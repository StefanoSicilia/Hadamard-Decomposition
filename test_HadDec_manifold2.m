    ...
    %'X',X,'Y',Y,'U',U,'V',V,...
    opts=struct('rank',r,'maxit',15000,'init','FS',...
        'c',1e-4,'beta',0,'safestop',15,'theta',2,'tau',1e-1);
    [X_FS,Y_FS,U_FS,V_FS,err_FS]=HadDec_manifold2(A,opts);
    fin_err_FS=err_FS(end);
    opts.init='Wertz';
    [X_Wertz,Y_Wertz,U_Wertz,V_Wertz,err_Wertz]=HadDec_manifold2(A,opts);
    fin_err_Wertz=err_Wertz(end);
    opts.init='FS2';
    tic;
    [X_FS2,Y_FS2,U_FS2,V_FS2,err_FS2]=HadDec_manifold2(A,opts);
    t_manifold=toc;
    fin_err_FS2=err_FS2(end);
    opts.init='FS3';
    [X_FS3,Y_FS3,U_FS3,V_FS3,err_FS3]=HadDec_manifold2(A,opts);
